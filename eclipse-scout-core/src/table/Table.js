...
_renderData() {
  ...
  if (Device.get().tableAdditionalDivRequired) {
...
_buildRowDiv(row) {
  ...
  var i, column,
    rowDiv = '<div class="' + rowClass + '" style="width: ' + rowWidth + 'px"' + Device.get().unselectableAttribute.string + '>';
...
resizeColumn(column, width) {
  ...
  if (visibleColumnIndex !== -1) {
    ...
    if (Device.get().tableAdditionalDivRequired) {
      ...
      // same calculation in Column.prototype.buildCellForRow;
    }
