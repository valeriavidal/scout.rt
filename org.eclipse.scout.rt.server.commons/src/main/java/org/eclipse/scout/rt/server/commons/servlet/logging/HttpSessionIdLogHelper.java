...
public class HttpSessionIdLogHelper {
  ...
  public String getSessionIdForLogging(HttpSession session) {
    ...
    session.setAttribute(SESSION_ATTRIBUTE, StringUtility.emptyIfNull(result));
    return result;
  }
  ...
  protected String getSuffix(String sessionId) {
    ...
    // Application servers sometimes seem to encode additional information in the session, such as the
    // target cluster node. Because the structure of session ids is not standardized, we have to "guess"
    // which part might be the most helpful. From looking at some examples, we have concluded that the
    // different parts often seem to be separated by a "dot" character:
    //
    // Examples:
    ...
    // identifies the cluster node?), dots are only considered after a reasonable distance from the beginning.
    // The number 15 was chosen simply because it is half of 30.
